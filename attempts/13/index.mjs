...
const draw_circle = (ctx) => {
    ...
    const sub_circ_count = Math.round(6 + Math.random() * 4)

    for (let x = (sub_circ_count - 1); x >= 0; x--) {
        ctx.beginPath()
        ctx.arc(
            center[0],
            center[1],
            x * (rad / sub_circ_count),
            0,
            360
        )
        x === (sub_circ_count - 1) && ctx.fill()
        ctx.stroke()
    }
}
...
const draw = (canvas) => {
    ...
    for (let x = 0; x < circ_count; x++) {
        const l = 30 + x / (circ_count / 100) * 0.7
